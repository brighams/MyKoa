...
export const valid_cdn_file = (db, {file_id, appid, file_name}) => {
  ...
  if (!is_positive_integer(file_id)
    ...
    const message = WRITE_LOG(`💥 💥 💥...ERROR: ILLEGAL PARAMETERS ${dbg_info({file_id, appid, file_name})}`)
    return {
      status: 422,
      valid: false,
      message
  ...
  if (Array.isArray(track) || track === undefined || track.dir_path === undefined) {
    const message = WRITE_LOG(`💥 💥 💥...ERROR: UNDEFINED ${dbg_info({file_id, appid, file_name, track})}}`)
    return {
      status: 404,
      valid: false,
      message,
  ...
  if (!is_valid_file(track.dir_path)) {
    const message = WRITE_LOG(`💥 💥 💥...ERROR: INVALID FULL PATH (track.dir_path) ${dbg_info({
      file_id,
      appid,
      file_name,
      track
  ...
  if (!is_valid_mime_type(mime_type)) {
    const message = WRITE_LOG(`💥 💥 💥...ERROR: INVALID MIME TYPE ${dbg_info({file_id, appid, file_name, track})}}`)
    return {
      status: 422,
      valid: false,
      message,
